"""
image_engine.py — WLDS-9 Image Inference via MobileNetV2 transfer learning.
Preprocessing: PIL load → resize (224, 224) → /255.0 → expand_dims → predict
Called by inference.py as: image_result = image_engine.run(image_path)
"""
...
def _load_model():
    ...
    if not os.path.exists(_LABELS_PATH):
        raise FileNotFoundError(f"[image_engine] labels.json not found at {_LABELS_PATH}")
    ...
    if not os.path.exists(weights_path):
        raise FileNotFoundError(f"[image_engine] Weights not found at {weights_path}")

    _model = _build_model(num_classes=len(_label_names))
    _model.load_weights(weights_path, skip_mismatch=True)
...
def run(image_path: str) -> dict:
    ...
    Returns species, confidence, habitat metadata, and raw scores.
    """
    _load_model()
    ...
    print(f"[image_engine] {common_name} ({scientific}) | conf={confidence:.4f} | coverage={body_coverage}%")
